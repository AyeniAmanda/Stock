...
import com.example.store.exception.NotFoundAlertException;
import com.example.store.repo.StockRepository;
        ...
        return fetchedStock
                .flatMap(stock -> {
                    if (ObjectUtils.isEmpty(stock)) {
                        return Mono.error(new NotFoundAlertException("Id has no record"));
                    }
                    StockDto stockDTO = mapToDto(stock);
                    return Mono.just(stockDTO);
                }).onErrorComplete(throwable -> throwable instanceof NotFoundAlertException);
    }
